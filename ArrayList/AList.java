	...
	public T remove(T item){
		T value = null;
			...
			for (int i = 0 ; i<this.size(); i++) {
				if(((Comparable)this.list[i]).compareTo((Comparable)item)==0) {
					value = this.list[i];
					i++;
	...
	public <T> int search(T item){
		...
		while(start<=end) {
			...
			if(this.list[mid].equals(item))
				return mid;
			else if (((Comparable)list[mid]).compareTo((Comparable)item)>0)
                end = mid - 1;
            else
                start = mid + 1;
	...
	public <object> void sort(){
        ...
        for (int i = 0; i < size()-1; i++) {
            for (int j = i + 1; j < size(); j++) {
                if (i != j) {
                    if (((Comparable) list[j]).compareTo((Comparable)list[i])<0) {
                        temp = (T) list[i];
                        list[i] = list[j];
                        list[j] = (T) temp;
                    } // Inner-if
