...
public class MinHeap<T extends Comparable<T>> {
    ...
    public void insert(T data) {
        ...
        if (root == null) {
            ...
            while (parent != null && node.getData().compareTo(parent.getData()) < 0) {
                swap(node, parent);
                node = parent;
    ...
    private void minHeapify(Node<T> node) {
        if (!isLeaf(node)) {
            ...
            if (leftChild != null && leftChild.getData().compareTo(smallest.getData()) < 0) {
                smallest = leftChild;
            }
            if (rightChild != null && rightChild.getData().compareTo(smallest.getData()) < 0) {
                smallest = rightChild;
            }
